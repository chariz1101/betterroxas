...
Pulls the latest posts from the official LGU Roxas City Facebook Page, categorizes
...
1. **Get an Editor role on the page.** Ask the LGU Roxas City page admin (via Meta
   Business Suite → Settings → People) to add the BetterRoxas account/app as an
...
| Name              | Type         | Value                                               |
| ----------------- | ------------ | --------------------------------------------------- |
| `FTP_SERVER`      | Secret       | cPanel FTP host (e.g. `ftp.betterroxas.vercel.app`)       |
